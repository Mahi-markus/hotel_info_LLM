...
from properties.models import RatingAndReview
    ...
    def process_hotel(self, model: Any, hotel_data: dict) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(
                f"Error processing hotel ID {hotel_data['id']}: {str(e)}"
            ))
            return None

    def truncate_ratings_table(self):
        """Ensure no duplicate primary key error by resetting the sequence"""
        try:
            with connection.cursor() as cursor:
                # Truncate the table and reset the auto-increment sequence for id column
                cursor.execute("TRUNCATE TABLE property_ratingandreview RESTART IDENTITY CASCADE;")
                self.stdout.write(self.style.SUCCESS("Successfully truncated property_ratingandreview table"))
        except Exception as e:
        ...
        try:
            ...
            for i in range(0, len(rows), self.BATCH_SIZE):
                ...
                for row in batch:
                    ...
                    # Skip hotel entries with empty or invalid hotel_id
                    if not hotel_data['id']:
                        self.stdout.write(self.style.WARNING(f"Skipping hotel with missing or invalid hotel_id"))
                        continue
                    ...
                    if result:
                        ...
                        # Save to database using `update_or_create` to avoid primary key conflicts
                        RatingAndReview.objects.update_or_create(
                            hotel_id=hotel_data['id'],  # Ensure hotel_id is used correctly
                            defaults={
                                'rating': float(generated_rating),
                                'review': review
                            }
                        )
